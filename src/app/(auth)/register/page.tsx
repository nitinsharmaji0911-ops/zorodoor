    ...
    <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center px-4 py-20">
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <h1 className="text-2xl font-black uppercase tracking-tight mt-6 mb-2 text-[#111]">Join The Cult</h1>
          <p className="text-[#888] font-medium text-sm">Get early access to limited drops and exclusive deals.</p>
        </div>

        <div className="bg-white border border-[#EAEAEA] rounded-3xl p-8 shadow-sm">
          {/* Google */}
          <button onClick={handleGoogleSignup} className="w-full flex items-center justify-center gap-3 border border-[#EAEAEA] rounded-xl py-3.5 font-bold text-sm text-[#111] hover:border-[#111] hover:bg-[#F9F9F9] transition-all mb-6">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            {error && (
              <div className="bg-[#FEE2E2] text-[#DC2626] text-xs font-bold p-3 rounded-xl">{error}</div>
            )}
        ...
        <p className="text-center text-sm font-medium text-[#888] mt-6">
          ...
          <Link href="/login" className="text-[#111] font-bold hover:text-[#FF3B30] transition-colors">Sign In</Link>
        </p>
      </div>
    </div>
  )
